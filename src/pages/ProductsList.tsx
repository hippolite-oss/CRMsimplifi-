...
import { 
    Search, Filter, Package, Edit, Trash2, Eye, 
    TrendingUp, TrendingDown, CheckCircle, XCircle,
    AlertTriangle, MoreVertical, Printer, Download,
    PlusCircle, RefreshCw, BarChart3, ShoppingBag,
    Hash, Tag, DollarSign, Box, Layers, ArrowUpDown,
    ChevronLeft, ChevronRight, Star, Grid, List,
    PackagePlus, Zap, Battery, AlertCircle
} from 'lucide-react';
...
const ProductsList = () => {
    ...
    const produitsMock = [
        ...
        },
        {
            ...
            tva: 18,
            statut: "actif",
            image: "https://images.unsplash.com/photo-1434493650001-5d43a6fea0a7?w=400",
            fournisseur: "Apple",
        ...
        },
        {
            ...
            tva: 18,
            statut: "actif",
            image: "https://images.unsplash.com/photo-1581344983112-6d4b4c59e11b?w=400",
            fournisseur: "Seagate",
    ...
    const categories = ['tous', 'Informatique', 'Téléphonie', 'Audio', 'Wearable', 'Accessoires'];
    ...
    const calculerStats = (produitsList) => {
        ...
        const lowStock = produitsList.filter(p => p.stock > 0 && p.stock <= p.stockMin).length;
        const outOfStock = produitsList.filter(p => p.stock === 0).length;
        const totalValue = produitsList.reduce((sum, p) => sum + (p.prixVente * p.stock), 0);
    ...
    const produitsFiltres = useMemo(() => {
        ...
        return filtered;
    }, [produits, search, filterCategorie, filterStock, filterStatus, sortBy, sortOrder]);
    ...
    const getStockStatus = (stock, stockMin) => {
        if (stock === 0) {
            return { color: 'bg-red-100 text-red-800', text: 'Rupture', icon: <XCircle className="w-4 h-4" /> };
        } else if (stock <= stockMin) {
            return { color: 'bg-yellow-100 text-yellow-800', text: 'Stock faible', icon: <AlertTriangle className="w-4 h-4" /> };
        } else {
            return { color: 'bg-green-100 text-green-800', text: 'En stock', icon: <CheckCircle className="w-4 h-4" /> };
        }
    };
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">{stats.total} produits référencés</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">{stats.totalValue.toLocaleString()} F</p>
                            </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {produitsPagines.map(produit => {
                            const stockStatus = getStockStatus(produit.stock, produit.stockMin);
                            return (
                                <div key={produit.id} className="bg-white rounded-lg shadow-sm border overflow-hidden hover:shadow-md transition-shadow">
                                    {/* Image */}
                                    <div className="relative h-48 overflow-hidden">
                                        ...
                                        <div className={`absolute top-2 left-2 px-2 py-1 rounded-full text-xs font-medium ${stockStatus.color} flex items-center gap-1`}>
                                            {stockStatus.icon}
                                            {stockStatus.text}
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <h3 className="font-bold text-gray-900 line-clamp-1">{produit.nom}</h3>
                                        ...
                                        <div className="mb-3">
                                            <p className="text-sm text-gray-600">{produit.categorie} • {produit.sousCategorie}</p>
                                        </div>
                                        
                                        <div className="grid grid-cols-2 gap-2 text-sm mb-4">
                                            <div>
                                                <p className="text-gray-600">Prix</p>
                                                <p className="font-bold text-blue-600">{produit.prixVente.toLocaleString()} F</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-600">Stock</p>
                                                <p className="font-bold">{produit.stock} {produit.unite}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p className={`text-sm font-bold ${getMargeColor(produit.marge)}`}>
                                                    {produit.marge}%
                                                </p>
                                            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                                ...
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            <input
                                                type="checkbox"
                                                checked={selectedProducts.length === produitsPagines.length && produitsPagines.length > 0}
                                    ...
                                    {produitsPagines.map(produit => {
                                        ...
                                        return (
                                            <tr key={produit.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="font-bold text-blue-600">{produit.prixVente.toLocaleString()} F</div>
                                                    <div className="text-xs text-gray-500">{produit.prixAchat.toLocaleString()} F achat</div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        <div className={`px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${stockStatus.color}`}>
                                                            {stockStatus.icon}
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    <div className="text-xs text-gray-500">{produit.sousCategorie}</div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className={`font-bold ${getMargeColor(produit.marge)}`}>
                                                        {produit.marge}%
                                                    </div>
                                                    <div className="text-xs text-gray-500">
                        ...
                        <div className="px-6 py-4 border-t flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div className="text-sm text-gray-700">
                                Affichage de {startIndex + 1} à {Math.min(endIndex, produitsFiltres.length)} sur {produitsFiltres.length} produits
                            </div>
                            <div className="flex items-center gap-2">
                                <button
                    ...
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full">
                        <div className="p-6">
                            ...
                            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                                <p className="font-medium text-red-900">Êtes-vous sûr de vouloir supprimer ce produit ?</p>
                                <p className="text-red-700 mt-1">{productToDelete.nom}</p>
                                <p className="text-sm text-red-600 mt-2">Code: {productToDelete.code}</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        {getStockStatus(selectedProduct.stock, selectedProduct.stockMin).icon && (
                                            <div className={`p-3 rounded-lg flex items-center gap-3 ${getStockStatus(selectedProduct.stock, selectedProduct.stockMin).color}`}>
                                                {getStockStatus(selectedProduct.stock, selectedProduct.stockMin).icon}
                                                <div>
                                                    <p className="font-medium">Statut stock</p>
                                                    <p>{getStockStatus(selectedProduct.stock, selectedProduct.stockMin).text}</p>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="p-3 bg-gray-50 rounded-lg">
                                            ...
                                            <p className="font-medium">
                                                {selectedProduct.statut === 'actif' ? 'Actif' : 'Inactif'}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="md:col-span-2">
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-3">
                                                <div>
                                                    ...
                                                    <p className="font-medium">{selectedProduct.categorie}</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm text-gray-600">Sous-catégorie</p>
                                                    <p className="font-medium">{selectedProduct.sousCategorie}</p>
                                                </div>
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <p className="font-medium">{selectedProduct.fournisseur}</p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div>
                                            <h3 className="font-semibold text-gray-900 mb-4">Prix et stock</h3>
                                            <div className="space-y-3">
                                                <div>
                                                    <p className="text-sm text-gray-600">Prix d'achat</p>
                                                    <p className="font-medium">{selectedProduct.prixAchat.toLocaleString()} F</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm text-gray-600">Prix de vente</p>
                                                    <p className="font-bold text-blue-600">{selectedProduct.prixVente.toLocaleString()} F</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm text-gray-600">Marge</p>
                                                    <p className={`font-bold ${getMargeColor(selectedProduct.marge)}`}>
                                    ...
                                    <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                                        <h4 className="font-semibold text-gray-900 mb-3">Gestion du stock</h4>
                                        <div className="grid grid-cols-3 gap-4">
                                            <div>
                                                <p className="text-sm text-gray-600">Stock actuel</p>
                                                <p className="text-xl font-bold">{selectedProduct.stock} {selectedProduct.unite}</p>
                                            </div>
                                            <div>
                                                <p className="text-sm text-gray-600">Stock minimum</p>
                                                <p className="text-lg font-medium">{selectedProduct.stockMin} {selectedProduct.unite}</p>
                                            </div>
                                            <div>
                                                <p className="text-sm text-gray-600">Stock maximum</p>
                                                <p className="text-lg font-medium">{selectedProduct.stockMax} {selectedProduct.unite}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {/* Performances */}
                                    <div className="mt-6">
                                        <h4 className="font-semibold text-gray-900 mb-3">Performances</h4>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="p-3 bg-blue-50 rounded-lg">
                                                <p className="text-sm text-gray-600">Ventes ce mois</p>
                                                <p className="text-xl font-bold text-blue-600">{selectedProduct.ventesMois}</p>
                                            </div>
                                            <div className="p-3 bg-green-50 rounded-lg">
                                                <p className="text-sm text-gray-600">Date d'ajout</p>
                                                <p className="text-lg font-medium text-green-600">{selectedProduct.dateAjout}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <footer className="mt-8 py-6 border-t bg-white">
                <div className="container mx-auto px-4">
                    <div className="text-center text-sm text-gray-600">
                        ...
                        <p className="mt-1">Valeur totale du stock: {stats.totalValue.toLocaleString()} F</p>
                    </div>
                </div>
            </footer>
        </div>
